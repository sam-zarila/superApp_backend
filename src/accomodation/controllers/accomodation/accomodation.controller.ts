...
export class AccomodationController {
  constructor(private readonly bookingService: AccomodationService) {}
  ...
  @Get('byselectedDate')
  @ApiOperation({ summary: 'Get booking transaction by selected date' })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({ summary: 'Get total amount for the current day' })
  @ApiResponse({
    status: 200,
  ...
  async updateOrdersById(
    ...
    await this.bookingService.updateBookingsById(id, updatedBookDto);
  ...
  @Post('checkout')
  @ApiOperation({ summary: 'Generate checkout URL for payment' })
  @ApiResponse({
    status: 201,
    description: 'Returns a checkout URL for payment processing',
  })
